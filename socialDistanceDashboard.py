...
import psutil
import argparse
from Barcode_Analogy import Barcode_heatmap_totalpeople,Barcode_heatmap_totalviolations,Barcode_heatmap_averageIntensity,\
    Barcode_heatmap_CPUUtilization,concateimages,Barcode_heatmap_RAMpercent
...
class SocialDistanceAnalysis:
    ...
    def converttogrey(self,img):
        B = img[:, :, 0]
        G = img[:, :, 1]
        R = img[:, :, 2]

        avgB = int(cv2.mean(B)[0])
        avgG = int(cv2.mean(G)[0])
        avgR = int(cv2.mean(R)[0])
...
parser = argparse.ArgumentParser(description="Barcode Analogy description")
parser.add_argument('--videoname', type=str, default='0', help='input video name')
parser.add_argument('--strip_width', type=str, default='5', help='Barcode each strip width')
parser.add_argument('--strip_height', type=str, default='50', help='Barcode each strip height')

args = parser.parse_args()
filename = args.videoname
barcode_each_strip_width = int(args.strip_width)
barcode_each_strip_height = int(args.strip_height)
...
Total_peopleviolating_SD_result = []
Total_people_result = []
Image_avg_intensity = []
CPUUtilization = []
RAMpercent = []
...
while(True):
    ...
    #SD Algorithm processing for every frame
    if (fno % 1 == 0 or fno == 1):
        AnalysisObj.SDASetup(yolopath, assetpath, videoShape)
        AnalysisObj.SDAProcess(currentImg)
        BGRAvg = AnalysisObj.converttogrey(currentImg)
        outputFrame = AnalysisObj.SDADisplay()

        risk_people = AnalysisObj.riskPeople
        total_people = AnalysisObj.Totalpeople
        cpu = psutil.cpu_percent(0.03)
        ram_percent = psutil.virtual_memory().percent

        Total_peopleviolating_SD_result.append(risk_people) #Total people in risk within people in frame
        Total_people_result.append(total_people)            #Total people in frame
        Image_avg_intensity.append(BGRAvg)                  #Avg intensity of frame
        CPUUtilization.append(cpu)                          #CPU Utilization
        RAMpercent.append(ram_percent)                      #percentage of used RAM


        cv2.putText(outputFrame,str(fno-1),(50,50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 3)
...
total_processed_frames = len(Total_peopleviolating_SD_result)

start_frame = 0
end_frame = total_processed_frames - 1
barcode_each_strip_width = 5 #pixels
barcode_height = 50 #pixels
violationsmap = Barcode_heatmap_totalviolations(start_frame,
                end_frame,
                Total_peopleviolating_SD_result,
                barcode_each_strip_width,
                barcode_each_strip_height,
                "Barcode_Violations.bmp")

peoplemap = Barcode_heatmap_totalpeople(start_frame,
                end_frame,
                Total_people_result,
                barcode_each_strip_width,
                barcode_each_strip_height,
...
avgintensitymap = Barcode_heatmap_averageIntensity(start_frame,
                  end_frame,
                  Image_avg_intensity,
                  barcode_each_strip_width,
                  barcode_each_strip_height,
                  "Barcode_Avgintensity.bmp")

CPUUtilizationmap = Barcode_heatmap_CPUUtilization(start_frame,
                                                   end_frame,
                                                   CPUUtilization,
                                                   barcode_each_strip_width,
                                                   barcode_each_strip_height,
                                                   "Barcode_CPUUtilization.bmp")

RAM_percent = Barcode_heatmap_RAMpercent(start_frame,
                                                   end_frame,
                                                   RAMpercent,
                                                   barcode_each_strip_width,
                                                   barcode_each_strip_height,
                                                   "Barcode_RAMpercent.bmp")


concateimages(violationsmap,peoplemap,avgintensitymap,CPUUtilizationmap,RAM_percent,"ConcateImage.bmp")
